...
function M.count_tokens_async(text, model_name, callback)
	...
	if #text > 50000 then
		...
		require("token-count.log").warn(
			string.format(
				"Content too large for Anthropic API (%d chars), returning estimate: %d tokens",
				#text,
				estimated_tokens
			)
		)
	...
	-- Get the path to the anthropic counter script
	local script_path = vim.fn.fnamemodify(debug.getinfo(1, "S").source:sub(2), ":p:h") .. "/anthropic_counter.py"
	...
	local job_id = vim.fn.jobstart(cmd, {
		...
		on_stderr = function(_, data)
			if data and data[1] and data[1] ~= "" then
				...
				callback(nil, "Anthropic error: " .. error_msg)
			end
		end,
		on_exit = function(_, exit_code)
			if exit_code ~= 0 then
				callback(nil, "Anthropic process failed with exit code: " .. exit_code)
			end
		end,
	})

	if job_id <= 0 then
		callback(nil, "Failed to start Anthropic counting job")
	end
end
...
function M.count_tokens_sync(text, model_name)
	...
	if result.code == 0 then
		...
		return nil, "Anthropic error: " .. (result.stderr or "Unknown error")
	end
end
...
function M.check_availability()
	...
	-- Check if anthropic library is available in the managed venv
	local venv = require("token-count.venv")
	local status = venv.get_status()
	if not status.ready then
	...
	if result.code ~= 0 or not result.stdout:match("OK") then
		return false, "anthropic library not installed in venv (will be installed automatically if needed)"
	end
