...
M.defaults = {
	...
	ignore_patterns = { -- Patterns to ignore during background processing
		"node_modules/.*",
		"%.git/.*",
		"%.svn/.*",
		"%.hg/.*",
	...
	cache = {
		...
		placeholder_text = "⋯",
		lazy_start = false, -- Start cache timer immediately
	},
}
...
local function validate_config(config)
	...
	if config.model then
		...
		if models_ok then
			...
			if not technical_name then
				table.insert(
					errors,
					string.format("Invalid model '%s'. Run :TokenCountModel to see available models.", config.model)
				)
			end
		end
	end
	...
	if config.cache then
		...
		if
			config.cache.max_files_per_batch
			and (type(config.cache.max_files_per_batch) ~= "number" or config.cache.max_files_per_batch < 1)
		then
