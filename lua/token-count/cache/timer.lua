...
--- Clean up expired cache entries for files not in buffers
function M.cleanup_expired_cache()
    local instance = require("token-count.cache.instance").get_instance()
    local now = vim.loop.hrtime() / 1000000
    local api = require("token-count.cache.api")
    
    local cleanup_count = 0
    for file_path, cache_entry in pairs(instance.cache) do
        if cache_entry.type == "file" then
            -- Only clean up files that are not in active buffers
            if not api._is_file_in_active_buffer(file_path) then
                if (now - cache_entry.timestamp) > instance.config.cache_ttl then
                    instance.cache[file_path] = nil
    ...
    if cleanup_count > 0 then
        log.info(string.format("Cleaned up %d expired cache entries", cleanup_count))
    end
end
