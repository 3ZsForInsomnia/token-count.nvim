...
function M.calculate_directory_tokens(dir_path, callback)
	...
	vim.schedule(function()
		...
		while name do
			...
			-- Check if should process file (with potential active buffer detection)
			-- print("--", vim.inspect(--))
			local should_process, _ = processor.should_process_file(full_path, false) -- false = not active for directory scanning
			if type == "file" and should_process then
...
function M.queue_directory_files(dir_path, recursive)
	...
	vim.schedule(function()
		...
		while name do
			...
			-- Check if should process file (always false for active since this is background directory scanning)
			local should_process, _ = processor.should_process_file(full_path, false)
			if type == "file" and should_process then
